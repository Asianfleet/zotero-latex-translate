...
7. 未运行的普通条目登记到 `taskRegistry` 并加入串行队列。
8. 插件对队列中的每个条目逐一处理。
9. 插件从当前条目字段中提取 arXiv。
10. 如果识别到 arXiv，构造 `--arxiv` 输入。
11. 如果未识别到 arXiv，弹窗要求用户为当前条目输入 TeX 源码链接或本地路径。
12. 插件分类当前条目的输入：
...
14. 插件启动外部命令。
15. 命令执行期间显示阶段状态。
16. 命令退出后，插件定位生成的 PDF。
17. 插件将 PDF 保存为当前条目的子附件。
18. 插件记录当前条目的成功、失败或跳过状态，并从 `taskRegistry` 释放该条目。
19. 插件继续队列。
20. 队列结束后显示批量汇总。
